...
def test_health_premium_uses_individual_young_for_age_under_35():
    young_profile = UserProfile(
        age=30, gender=Gender.FEMALE, city_tier=CityTier.TIER_1,
        annual_income=10_00_000, monthly_expenses=30_000,
        monthly_insurance_budget=5_000, tax_regime=TaxRegime.NEW,
    )
...
def test_health_premium_uses_individual_senior_for_age_over_45():
    senior_profile = UserProfile(
        age=50, gender=Gender.MALE, city_tier=CityTier.TIER_1,
        annual_income=20_00_000, monthly_expenses=60_000,
        monthly_insurance_budget=15_000, tax_regime=TaxRegime.NEW,
    )
...
def test_disability_premium_is_always_zero(family_profile):
    assert estimate_monthly_premium(PolicyType.DISABILITY, 9_00_000, family_profile) == 0.0
...
def test_health_first_override_does_not_trigger_with_existing_health_cover(family_profile):
    life_need, health_need, ad_need = _needs(health_existing=5_00_000)
    plan = build_action_plan(life_need, health_need, ad_need, [], family_profile)
    types = _ordered_types(plan)
    assert types.index(PolicyType.TERM) < types.index(PolicyType.HEALTH)
...
def test_no_dependents_override_accident_before_term_when_health_covered(single_profile):
...
def test_budget_split_term_within_health_beyond():
    profile = UserProfile(
        ...
        annual_income=18_00_000, monthly_expenses=50_000,
